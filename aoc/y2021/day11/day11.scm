...
(define-module (aoc y2021 day11 day11)
  ...
  #:export (parse-line parse-lines flashed? mark-flashed reset-flashed
		       build-neighbors flash step day11-part1 day11-part2))
...
(define (flashed? flashed item)
  (and
   (hashtable-contains? flashed item)
   (hashtable-ref flashed item #f)))

;; Mark an item as visited
(define (mark-flashed flashed item)
  (hashtable-set! flashed item #t))

;; Reset all the items that flashed this step to zero
(define (reset-flashed arr flashed i-max j-max)
  (begin
    (for-each
     (lambda (idx)
       (array-set! arr 0 (first idx) (second idx)))
     (vector->list (hashtable-keys flashed)))))

;; Cache neighbor list into an array of the same dimensions as the
;; board.
(define (build-neighbors arr i-max j-max)
  (let ((dst (make-array 0 (list 0 i-max) (list 0 j-max))))
    (begin
      (array-index-map!
       dst
       (lambda (i j)
	 (find-neighbors-with-diag i j i-max j-max)))
      dst)))


;; Flash an octopus, incrementing its neighbors
(define (flash arr neighbor-array flashed item)
  (let ((i (first item))
	(j (second item)))
    (if (not (flashed? flashed item))
	(let ((neighbors (array-ref neighbor-array i j)))
	  (begin
	    (mark-flashed flashed item)
	    ;; Increment every neighbor
	    (for-each
	     (lambda (x)
	       (array-set! arr (1+ (array-ref arr (first x) (second x)))
			   (first x) (second x)))
	     neighbors)
	    (list 1 (filter
		 (lambda (x)
		   (> (array-ref arr (first x) (second x)) 9))
		 neighbors))))
	(list 0 '()))))

;; An individual step for this problem
;; Increments every octopus by one.
;; Then find octopuses that flashed and increments the neighbors.
(define (step arr neighbor-array i-max j-max)
  (let ((flashed (make-hashtable (lambda (x) (hash x 10000)) equal?)))
    (begin
      ;; First, increase every cell by 1
      (array-map! arr (lambda (x) (1+ x)) arr)
      ;; Then flash any octopus with an energy level greater than 9.
      (let update-while-gt9 ((update? #t)
			     (queue
			      (filter
			       (lambda (index)
				 (> (array-ref arr (first index) (second index)) 9))
			       (apply append (array->list (array-indexes arr))))))
	(begin
	  (if (not (null? queue))
	      (let* ((index (car queue))
		     (queue (cdr queue))
		...
		(begin
		  (if (> (array-ref arr i j) 9)
		      (begin
			(let ((neighbors (flash arr neighbor-array flashed (list i j))))
			  (begin
			    (set! queue (append (second neighbors) queue))
	 		    (update-while-gt9 update? queue))))
	 	      (update-while-gt9 update? queue)))))))
      (reset-flashed arr flashed i-max j-max))
    (hashtable-size flashed)))


;; Compute part 1 for day 11
(define (day11-part1 filename)
  (let ((lines (parse-lines (get-lines filename))))
    (let ((neighbor-array (build-neighbors lines 9 9)))
      (let lp ((x 100) (num-flashes 0))
	(if (positive? x)
	    (lp (1- x) (+ num-flashes (step lines neighbor-array 9 9)))
	    (display (format #f "day 11 part 1 final-value: ~a\n" num-flashes)))))))


;; Compute part 2 for day 11
(define (day11-part2 filename)
  (let ((lines (parse-lines (get-lines filename))))
    (let ((neighbor-array (build-neighbors lines 9 9)))
      (let lp ((num-steps 0) (num-flashes 0))
	(if (and (not (= num-flashes 100))
		 (< num-steps 1000))
	    (lp (1+ num-steps) (step lines neighbor-array 9 9))
	    (display (format #f "day 11 part 2 final-value: ~a\n" num-steps)))))))

;; The parts of the day that should get run
(define parts (list day11-part1 day11-part2))
